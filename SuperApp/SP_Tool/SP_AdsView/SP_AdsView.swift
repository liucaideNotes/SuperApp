...
extension SP_AdsView {
     ...
     * 对外接口
     */
    class func show(_ view:UIView ,frame: CGRect,  imageUrls: [String], time: TimeInterval = 5.0, adsType: SP_AdsViewType = .default_H, itemSize:CGSize = CGSize(width: 0, height: 0), pageAlignment:UIPageControl.PageControlAlignmentType = .center) -> SP_AdsView {
        for subView in view.subviews {
            if let adsView = subView as? SP_AdsView {
                adsView.frame = frame
                adsView.collectionView.frame.size.height = frame.size.height
                adsView.scrollPageControl.frame.origin.y = frame.size.height - 20
                switch adsType {
                case .default_H:
...
class SP_AdsView: UIView {
    ...
    /// 默认的图片
    static var placeholderImage = "HuanChong"
    
    ///闭包传值
    var SP_AdsViewClosures: ((_ itemIdex: Int, _ isSelect:Bool) -> Void)?
    ///collectionView
    lazy var collectionView: UICollectionView = {
        ...
        view.register(UINib(nibName: "SP_AdsColleCell", bundle: nil), forCellWithReuseIdentifier: "SP_AdsColleCell")
        return view
    }()
    ...
    lazy var scrollPageControl: UIPageControl = {
    ...
    fileprivate func changePageValue() {
        ...
        self.SP_AdsViewClosures?(itemIdex % self._imageUrls.count, false)
    }
...
extension SP_AdsView: UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "SP_AdsColleCell", for: indexPath) as! SP_AdsColleCell
        
        switch _adsType {
        case .default_H,.half_H:
            cell.imageName = _imageUrls[indexPath.item % _imageUrls.count]
        default:
            cell.imageName = _imageUrls[indexPath.item % _imageUrls.count]
        }
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        SP_AdsViewClosures?(indexPath.item % _imageUrls.count, true)
    }
    //开始拖曳
