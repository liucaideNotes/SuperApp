...
- (void)cameraBackgroundDidClickPlay {
    ...
    if (![self.videoOutPut isRecording]) {
        ...
        // LCD ==> --------
//        NSURL *url = [[SP_FileManager defaultManager] filePathUrlWithUrl:[self getVideoSaveFilePathString]];
//        
//        NSString *documentDir= [NSSearchPathForDirectoriesInDomains ( NSDocumentDirectory , NSUserDomainMask , YES ) objectAtIndex : 0 ];
//        //保存首页图片 写真背景图
//        NSString *customFolderPath = [documentDir stringByAppendingPathComponent:@"tempVideo.mp4"];
//        NSURL *fileURL = [[NSURL alloc]initFileURLWithPath:customFolderPath];
...
- (NSURL *)outPutFileURL
{
    return [NSURL fileURLWithPath:[NSString stringWithFormat:@"%@%@", NSTemporaryDirectory(), [NSString stringWithFormat:@"hx%@.mp4",[self videoOutFutFileName]]]];
}
...
{
    self.videoTime++;
    [self.progressView setProgress:self.videoTime / 20.f animated:YES];
    if (self.videoTime == 20) {
        [timer invalidate];
        [self.videoBtn setImage:[UIImage imageNamed:@"camera_video_background@2x.png"] forState:UIControlStateNormal];
        [self.videoOutPut stopRecording];
    }
