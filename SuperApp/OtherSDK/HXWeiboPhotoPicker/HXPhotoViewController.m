...
- (void)setup
{
    ...
    if (self.manager.selectedPhotos.count > 0 && self.manager.isOriginal) {
        ...
        //bottomView.originalBtn.hidden = YES;
        ...
        if (self.manager.type == HXPhotoManagerSelectedTypePhotoAndVideo) {
            ...
            [[AVAudioSession sharedInstance] requestRecordPermission:^(BOOL granted) {
                if (granted) {
                    ...
                    NSLog(@"麦克风关闭了");
                    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"没有开启麦克风" message:@"" delegate:self cancelButtonTitle:@"不用麦" otherButtonTitles:@"去开启", nil];
                    [alert show];
...
- (void)cellDidSelectedBtnClick:(HXPhotoViewCell *)cell Model:(HXPhotoModel *)model
{
    if (!cell.selectBtn.selected) { // 弹簧果冻动画效果
        ...
        if (model.type == HXPhotoModelMediaTypeVideo) {
            ...
            if (model.asset.duration > 20) {
                [self.view showImageHUDText:@"视频大于20秒,暂不支持"];
                return;
            }
        }
...
- (void)changeButtonClick:(HXPhotoModel *)model
{
    ...
    [self.bottomView didUpClick:self.bottomView.originalBtn];
}
...
- (void)changeOriginalState:(BOOL)selected IsChange:(BOOL)isChange
{
    if (selected) { // 选中时
        if (isChange) { // 改变成初始状态
            ...
            CGFloat originalBtnX = self.bottomView.originalBtn.frame.origin.x;
            CGFloat originalBtnY = self.bottomView.originalBtn.frame.origin.y;
            CGFloat originalBtnW = 65;//self.bottomView.originalBtn.frame.size.width + 50;
            CGFloat originalBtnH = self.bottomView.originalBtn.frame.size.height;
            self.bottomView.originalBtn.frame = CGRectMake(originalBtnX, originalBtnY, originalBtnW, originalBtnH);
        }
        // 记录原图按钮的frame
        ...
        CGFloat originalBtnX = self.bottomView.originalBtn.frame.origin.x;
        CGFloat originalBtnY = self.bottomView.originalBtn.frame.origin.y;
        CGFloat originalBtnW = 110;//self.bottomView.originalBtn.frame.size.width + 50;
        CGFloat originalBtnH = self.bottomView.originalBtn.frame.size.height;
        self.bottomView.originalBtn.frame = CGRectMake(originalBtnX, originalBtnY, originalBtnW , originalBtnH);
        __weak typeof(self) weakSelf = self;
        // 获取一组图片的大小
        [HXPhotoTools FetchPhotosBytes:self.manager.selectedPhotos completion:^(NSString *totalBytes) {
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.25 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                ...
                weakSelf.bottomView.originalBtn.frame = CGRectMake(originalBtnX, originalBtnY, originalBtnW, originalBtnH);
            });
        }];
    }else {// 取消选中 恢复成初始状态
...
- (void)didUpClick:(UIButton *)button
{
