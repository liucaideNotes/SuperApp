...
@property (nonatomic, strong) SP_MBProgressHUD *HUD;
...
- (void)readyCancelSound {
    ...
    _textLable.font = [UIFont boldSystemFontOfSize:14.0];
...
- (void)initHUBViewWithView:(UIView *)view {
    ...
    if (_HUD == nil) {
        _HUD = [[SP_MBProgressHUD alloc] initWithView:view];
        _HUD.opacity = 0.4;
        ...
        _HUD.mode = SP_MBProgressHUDModeCustomView;
    }
    if ([view isKindOfClass:[UIWindow class]]) {
        [view addSubview:_HUD];
    } else {
...
- (void)startRecord {
    ...
    //设置文件保存路径和名称
    NSString *fileName = [NSString stringWithFormat:@"/voice-%.0f.caf", [[NSDate date] timeIntervalSince1970]*1000 ];
